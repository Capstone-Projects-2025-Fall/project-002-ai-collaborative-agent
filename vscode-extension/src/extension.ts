...
export async function activate(context: vscode.ExtensionContext) {
    ...
    async () => {
      ...
      if (!activeProjectId) {
        vscode.window.showWarningMessage(
          "No active project selected. Please select a project first in the AI Collab panel."
        );
        return;
      }
      ...
      const confirm = await vscode.window.showInformationMessage(
        `Index workspace for RAG? This will scan code files and generate embeddings.`,
        "Index Now",
        "Cancel"
      );
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          const success = await ragService.indexWorkspace(
            activeProjectId,
          ...
          if (success) {
            ...
            vscode.window.showInformationMessage(
              "✓ Workspace indexed! RAG enabled for AI features."
            );
          } else {
  ...
  const toggleRAGCmd = vscode.commands.registerCommand(
    "aiCollab.toggleRAG",
    async () => {
      const activeProjectId = extensionContext.globalState.get<string>(ACTIVE_PROJECT_KEY);
      if (!activeProjectId) {
      ...
      vscode.window.showInformationMessage(
        `RAG ${newStatus ? "enabled" : "disabled"} for AI features.`
      );
    }
  );
  context.subscriptions.push(toggleRAGCmd);

  const ragStatsCmd = vscode.commands.registerCommand(
    "aiCollab.ragStats",
    async () => {
      const activeProjectId = extensionContext.globalState.get<string>(ACTIVE_PROJECT_KEY);
      if (!activeProjectId) {
      ...
      vscode.window.showInformationMessage(
        `RAG Status: ${stats.enabled ? "Enabled" : "Disabled"}\n` +
  ...
  try {
    ...
    ragService = new RAGService(supabaseClient, getUserId);
  } catch (error) {
    vscode.window.showErrorMessage(
...
If you cannot provide JSON, provide the response in the numbered format as before, and I will parse it.`;
        ...
        try {
          ...
          let ragContext = '';
          let ragStats: RAGStats | null = null;
          ...
          if (ragContext) {
            enhancedPromptContent = promptContent.replace(
              '=== AI ANALYSIS REQUEST ===',
              `${ragContext}\n=== AI ANALYSIS REQUEST ===`
            );
          }
          ...
          vscode.window.showInformationMessage(
            `✅ AI analysis complete for project: ${projectToPrompt.name}${ragInfo}`
          );
        } catch (error: any) {
          vscode.window.showErrorMessage(

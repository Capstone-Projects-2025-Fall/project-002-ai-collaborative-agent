...
function activateCodeReviewer(context) {
    try {
        ...
        context.subscriptions.push(analyzeCommand, showResultsCommand, configureCommand, toggleAutoAnalyzeCommand, saveListener);
        updateStatusBar();
...
function startAutoAnalyze() {
    ...
    const intervalMinutes = config.get('autoAnalyzeInterval', 5);
...
function showResultsPanel(analysisResult, filesSummary, folderPath, fileCount) {
    ...
    resultsPanel = vscode.window.createWebviewPanel('aiCodeReviewResults', 'AI Code Review Results', 
...
function updateResultsPanel(analysisResult, filesSummary, folderPath, fileCount) {
    if (resultsPanel) {
        resultsPanel.webview.html = getWebviewContent(analysisResult, filesSummary, folderPath, fileCount);
    }
}
function getWebviewContent(analysisResult, filesSummary, folderPath, fileCount) {
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>

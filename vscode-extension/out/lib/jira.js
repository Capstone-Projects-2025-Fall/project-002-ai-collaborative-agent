...
async function jiraRequest(auth, path, init = {}) {
    ...
    return (await response.json()); // Parse JSON payload into typed response
}
...
async function getAssignableUsers(auth, projectKey) {
    // Fetch Jira users who can be assigned issues in the given project
    const users = await jiraRequest(auth, `/rest/api/3/user/assignable/search?project=${encodeURIComponent(projectKey)}`);
    return (users || []).map((u) => ({
